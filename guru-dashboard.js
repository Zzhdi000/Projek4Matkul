...
// Ambil data dari localStorage (harus sudah di-set saat login)
...
// Jika wali kelas, disable dropdown dan tambahkan info
...
window.tambahSiswa = async function() {
    ...
    if (guruwaliKelas) defaultKelas = guruwaliKelas;
...
const tabDataDiri = document.getElementById("tab-data-diri");
const contentDaftar = document.getElementById("content-daftar");
const contentDataDiri = document.getElementById("content-data-diri");

function switchToTab(tab) {
    if (tab === "daftar") {
        tabDaftar.className = "px-8 py-4 text-sm font-bold border-b-2 border-blue-600 text-blue-600 bg-white transition-all";
        tabDataDiri.className = "px-8 py-4 text-sm font-bold text-slate-400 hover:text-slate-600 transition-all";
        contentDaftar.classList.remove("hidden");
        contentDataDiri.classList.add("hidden");
    } else {
        tabDataDiri.className = "px-8 py-4 text-sm font-bold border-b-2 border-blue-600 text-blue-600 bg-white transition-all";
        tabDaftar.className = "px-8 py-4 text-sm font-bold text-slate-400 hover:text-slate-600 transition-all";
        contentDataDiri.classList.remove("hidden");
        contentDaftar.classList.add("hidden");
    }
}
tabDaftar.addEventListener("click", () => switchToTab("daftar"));
tabDataDiri.addEventListener("click", () => switchToTab("data-diri"));
