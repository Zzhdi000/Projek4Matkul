    ...
    // ==================== FIRESTORE & CRUD SISWA (DENGAN PAGING - TIDAK MENGHAPUS APAPUN) ====================
    function escapeHtml(str) {
        if (!str) return '';
        return str.replace(/[&<>]/g, m => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;' }[m]));
    }

    // FUNGSI loadStudents ASLI (sudah dimodifikasi menjadi paging, namun tetap dipanggil dengan cara yang sama)
    // Agar tidak merusak pemanggilan yang ada, kita ganti isinya dengan paging.
    async function loadStudents() {
        // Fungsi ini sekarang memanggil loadStudentsPage(true)
    ...
    async function loadStudentsPage(reset = true, direction = 'next') {
        ...
        // Tampilkan loading
        bodyDaftar.innerHTML = `<tr><td colspan="4" class="text-center py-10 text-slate-400"><i class="fas fa-spinner fa-pulse mr-2"></i> Memuat data......</td></tr>`;
        bodyDataDiri.innerHTML = `<tr><td colspan="6" class="text-center py-10 text-slate-400"><i class="fas fa-spinner fa-pulse mr-2"></i> Memuat data......</td></tr>`;
        ...
        try {
            ...
            if (guruData.waliKelas) {
                ...
                selectedKelas = filterKelasSelect ? filterKelasSelect.value : "all";
            }
            ...
            if (reset) {
                ...
                if (direction === 'next' && lastDoc) {
                    ...
                    Swal.fire("Info", "Tombol sebelumnya dinonaktifkan karena Firestore tidak mendukung offset. Gunakan tombol Selanjutnya.", "info");
                    isLoading = false;
                    return;
                } else {
                    isLoading = false;
            ...
            if (docs.length === 0 && reset === false && direction === 'next') {
            ...
            if (docs.length > 0) {
                ...
                firstDoc = docs[0];
                if (!reset && direction === 'next') currentPage++;
    ...
    function renderStudentTables(docs) {
        ...
        if (docs.length === 0) {
            const emptyRow = `<td><td colspan="6" class="p-8 text-center text-slate-400 italic">Tidak ada data untuk kelas ini</td></tr>`;
        ...
        docs.forEach(doc => {
            const v = doc.data();
            const id = doc.id;
            htmlDaftar += `<tr class="hover:bg-slate-50/80 transition animate-fadeIn border-b border-slate-100">
            ...
            htmlDataDiri += `<tr class="bg-white border border-slate-100 shadow-sm rounded-2xl hover:shadow-md transition-all animate-fadeIn">
   ...
   function updatePaginationControls(loadedCount, kelasFilter) {
    // Cari elemen card utama tempat kita akan menaruh pagination
    ...
    if (!paginationDiv) {
        ...
        // Masukkan setelah div dengan class "p-8" di dalam card
        const contentDiv = mainCard.querySelector('.p-8');
        if (contentDiv) {
            contentDiv.insertAdjacentElement('afterend', paginationDiv);
        } else {
            mainCard.appendChild(paginationDiv);
        }
    }
    ...
    paginationDiv.innerHTML = `
        <button id="btn-prev" class="px-6 py-2 bg-white border border-slate-200 rounded-xl text-sm font-bold text-slate-600 hover:bg-slate-50 disabled:opacity-50 disabled:cursor-not-allowed" ${currentPage === 1 ? 'disabled' : ''}>
            <i class="fas fa-chevron-left mr-2"></i> Sebelumnya
        </button>
        <span class="text-sm font-bold text-slate-500 bg-slate-100 px-4 py-2 rounded-full">Halaman ${currentPage}</span>
        <button id="btn-next" class="px-6 py-2 bg-white border border-slate-200 rounded-xl text-sm font-bold text-slate-600 hover:bg-slate-50 disabled:opacity-50 disabled:cursor-not-allowed" ${loadedCount < pageSize ? 'disabled' : ''}>
    ...
    if (btnPrev) btnPrev.onclick = () => loadStudentsPage(false, 'prev');
    if (btnNext) btnNext.onclick = () => loadStudentsPage(false, 'next');
}
    ...
    function handleImportExcel(file) {
        ...
        reader.onload = function(e) {
            ...
            Promise.all(promises).then(() => { Swal.fire("Berhasil!", `Data diimpor ke Kelas ${defaultKelas}`, "success"); loadStudentsPage(true); }).catch(err => Swal.fire("Error", "Gagal impor", "error"));
        };
    ...
    if (filterKelasSelect && !guruData.waliKelas) {
        filterKelasSelect.addEventListener("change", () => loadStudentsPage(true));
    }
    ...
    // Panggil loadStudents yang sudah diarahkan ke paging
    loadStudents();